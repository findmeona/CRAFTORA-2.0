(() => {
  ...
  /* ---------- Scroll reveal ---------- */
  // Hero content is intentionally excluded so the page opens straight into
  // the main content instead of fading in on load.
  const revealTargets = document.querySelectorAll(
    '.section-head, .cat-card, .story-card, .process-list li, .editor-inner, .gallery-card, .gallery-filters'
  );
  ...
  /* ---------- Editor mock: live text editing ---------- */
  const tryEditorBtn = document.getElementById('tryEditorBtn');
  const canvasHeadline = document.getElementById('canvasHeadline');
  const canvasDate = document.getElementById('canvasDate');
  const layerHeadlineLabel = document.getElementById('layerHeadlineLabel');

  if (tryEditorBtn && canvasHeadline) {
    tryEditorBtn.addEventListener('click', () => {
      canvasHeadline.scrollIntoView({ behavior: 'smooth', block: 'center' });
      canvasHeadline.focus();
      const range = document.createRange();
      range.selectNodeContents(canvasHeadline);
  ...
  if (canvasHeadline && layerHeadlineLabel) {
    canvasHeadline.addEventListener('input', () => {
      const text = canvasHeadline.textContent.trim().replace(/\s+/g, ' ') || 'Headline';
      layerHeadlineLabel.textContent = `Headline — "${text}"`;
    });
    canvasHeadline.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); canvasHeadline.blur(); }
    });
  }
  if (canvasDate) {
    canvasDate.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); canvasDate.blur(); }
    });
  }
  ...
  document.querySelectorAll('.gallery-demo-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const bgFromVisual = visual && visual.style.backgroundImage
        ? visual.style.backgroundImage.slice(5, -2)
        : null;
      openLightbox({
        ...
        visualEl: visual,
        photoUrl: btn.dataset.photo || bgFromVisual,
  ...
  function updateGalleryCount() {
    ...
    const visible = Array.from(galleryCards).filter(c => !c.classList.contains('is-hidden')).length;
    galleryCount.textContent = `${visible} design${visible === 1 ? '' : 's'}`;
  }
  ...
  galleryFilters.forEach(filterBtn => {
    filterBtn.addEventListener('click', () => {
      ...
      setTimeout(updateGalleryCount, 260);
    });
  });
  updateGalleryCount();

  if (gallerySort && galleryGrid) {
    gallerySort.addEventListener('change', () => {
      ...
      const priceOf = (c) => {
        const t = c.querySelector('.gallery-price');
        return t ? parseFloat(t.textContent.replace(/[^0-9.]/g, '')) || 0 : 0;
      };
      const ratingOf = (c) => {
        const t = c.querySelector('.gallery-rating small');
        return t ? parseFloat(t.textContent) || 0 : 0;
      };

      if (mode === 'price-low') cards.sort((a, b) => priceOf(a) - priceOf(b));
      else if (mode === 'price-high') cards.sort((a, b) => priceOf(b) - priceOf(a));
      else if (mode === 'rating') cards.sort((a, b) => ratingOf(b) - ratingOf(a));
      // 'popular' keeps the existing DOM order as-is
